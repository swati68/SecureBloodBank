        ...
        if($_SERVER["REQUEST_METHOD"] == "POST"){
            ...
            $bank_name = test_input(mysqli_real_escape_string($con,$_POST['bb_name']));
            if($quantity<=0){
                $err1="Please enter a Valid Quantity";
            }
            else{
                $stmt=$con->prepare("INSERT INTO blood (bgp,ant,qty,bank) VALUES (?,?,?,?)");
                $stmt->bind_param("ssss",$grp,$ant,$quantity,$bank_name);
            //$e = "INSERT INTO blood (bgp,ant,qty,bank) VALUES ('$grp','$ant','$quantity','$bank_name')";
            //mysqli_select_db($conn,'isaa_project');
            //$res = mysqli_query($con,$e);
            $res=$stmt->execute();
            if($res){
                header("location:b_add_done.php");
            }
            else{
        ...
        <div style='background-color:blue;'>
            <nav align='right'>
                <ul style='list-style-type:none;'>
                    <li style='display:inline-block; margin-right:15px;'><a href='blood_view.php' style="font-size:1.3em; padding-bottom:3px;text-decoration:none;"><font color="white">Back</font></a></li>
                </ul>
            </nav>
        </div>
        <div style="background-color:blue;">
        ...
        <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" align="center" style="font-size:20px;">
            <div style="border-style:groove; height:300px; width:700px; margin:auto; border-color:lightgrey;">
                ...
                <table align="center" border="0" style="border-collapse:collapse; width:80%; padding:50px">
                    ...
                    <tr>
                        <td style = "text-align:center; padding:10px 20px;">Blood Bank Name:</td>
                        <td style="text-align:center; padding:10px 20px;"><input type="text" name="bb_name"></td>
                    </tr>
                    <tr>
